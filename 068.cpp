...
int main() {
	int n,q;
	IN(n,q);
	vector< tuple<int, int,int,LL> > query(q);
	IN(query);
	// 辺の接続情報
	vector<LL> edge(n-1,-1);
	for(auto &[t,x,y,v]:query){
		x--,y--;
		if(t==0) edge[x]=v;
	}
	// 値の仮埋め
	vector<LL> values(n, 0); // 無い値は0と仮定して埋める
	REP(i, n-1)
		values[i+1] = edge[i]-values[i];
	// 連結確認用集合
	set<int> link;
	REP(i, n)
		link.insert(i);
	for(const auto &[t,x,y,v]:query){
		if(t==0) link.erase(x);
		else{
			int m=min(x,y), M=max(x,y);
			if(*link.lower_bound(m)<M){// 連結確認
				OUT("Ambiguous");
				continue;
			}
			LL diff = v-values[x];
			OUT((x-y)%2 ? values[y]-diff : values[y]+diff);
		}
	}
