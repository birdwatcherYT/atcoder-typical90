...
int select_best_cols(const VVI &mat, int select_rows){
	vector<MII> cnt(mat[0].size());
	int selected_rows=0;
	REP(i, SZ(mat)){
		if((1<<i)&select_rows){
			selected_rows++;
	...
	REP(j, SZ(mat[0])){
		if(cnt[j].size()==1)
			ans[cnt[j].begin()->first]++;
	}
	int ans_max=0;
	for(auto [k,v]:ans)
		ans_max=max(ans_max, v);
	return ans_max*selected_rows;
}
...
int main() {
	...
	int ans=0;
	REP(select_rows, 1<<h)
		ans=max(ans, select_best_cols(P, select_rows));
