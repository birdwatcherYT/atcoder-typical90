...
int main() {
	...
	FOR(i, 1, n+1)
		cin>>A[i]>>B[i];
	const int upper= 100001;
	vector<vector<bool>> dp(n+1, vector<bool>(upper, false));
	dp[0][0]=true;
	FOR(i, 1, n+1){
		REP(j, upper)
			dp[i][j]=(j>=A[i] && dp[i-1][j-A[i]]) || (j>=B[i] && dp[i-1][j-B[i]]);
	}
	if (!dp[n][s]){
		OUT("Impossible")
		return 0;
	...
	int value=s;
	for(int i=n; i>=1; --i){
		if (value>=A[i] && dp[i-1][value-A[i]]){
			ans="A"+ans;
			value-=A[i];
			continue;
		}
		if (value>=B[i] && dp[i-1][value-B[i]]){
			ans="B"+ans;
			value-=B[i];
			continue;
		}
