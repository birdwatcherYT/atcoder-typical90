...
int main() {
	...
	// 横方向の累積和
	FOR(i,0,LEN+1)FOR(j,1,LEN+1)
		cord[i][j]+=cord[i][j-1];
	// 縦方向の累積和
	FOR(j,0,LEN+1)FOR(i,1,LEN+1)
		cord[i][j]+=cord[i-1][j];
	int maxval = 0;
	FOR(i,1,LEN+1-k)FOR(j,1,LEN+1-k){
		maxval=max(maxval, cord[i+k][j+k]-cord[i-1][j+k]-cord[i+k][j-1]+cord[i-1][j-1]);
	}
	OUT(maxval);
	return 0;
}
