...
// [left, right)
vector< vector<LL> > solve(const vector<LL>& a, int k, LL p, int left, int right){
	// 個数 -> 価格
...
int main() {
	...
	int mid=n/2;
	auto ans1=solve( a, k, p, 0, mid);
	auto ans2=solve( a, k, p, mid, n);
	...
	// 結果を統合
	REP(cnt1, k+1){
		for (LL price1: ans1[cnt1]){
			LL upper=p-price1;
			LL cnt2=INSPOS(ans2[k-cnt1], upper+1);
			ans+=cnt2;
		}
	}
	OUT(ans)
