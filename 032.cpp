...
int run(const VVI &A, const vector< vector<bool> > &ng, VI &ord, vector<bool> &visit){
	int n = SZ(A);
	int ans=INT_MAX;
	if (SZ(ord)==n){
		ans=0;
	...
	REP(i, n){
		if (visit[i])
			continue;
		if (ord.size()!=0 && ng[ord[ord.size()-1]][i])
			continue;
		visit[i]=true;
		ord.push_back(i);
		ans = min(ans, run(A, ng, ord, visit));
		ord.pop_back();
		visit[i]=false;
	}
	return ans;
...
int main() {
	...
	int ans = run(A, ng, ord, visit);
	OUT(ans!=INT_MAX? ans : -1)
	return 0;
}
