...
int main() {
	...
	vector< vector<bool> > wall(h+2, vector<bool>(w+2, false));
	// 周りを壁で囲む
	REP(i, h+2)
		wall[i][0]=wall[i][w+1]=true;
	REP(j, w+2)
		wall[0][j]=wall[h+1][j]=true;
	...
	vector< VVI > dist(4, VVI(h+2,VI(w+2, h*w)));

	const vector<PII> d={{1,0},{-1,0},{0,1},{0,-1}};
	deque<tuple<int,int,int,int>> q;
	REP(i,4){
		dist[i][start.first][start.second]=0;
		int x=start.first+d[i].first, y=start.second+d[i].second;
		q.push_back({0, i, x, y});
	}
	while(!q.empty()){
		auto [val, i, x, y]=q.front();
		q.pop_front();
		if (!wall[x][y] && dist[i][x][y]>val){
			dist[i][x][y]=val;
			REP(j,4){
				if (i==j)
					q.push_front({val, j, x+d[j].first, y+d[j].second});
				else
					q.push_back({val+1, j, x+d[j].first, y+d[j].second});
			}
		}
	}
	int ans=h*w;
