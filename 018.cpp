...
int main() {
	...
	REP(i, q){
		...
		double theta=-_PI/2-2*_PI*e/t;
		double pos_y=l*cos(theta)/2;
		double pos_z=l/2.0 + l*sin(theta)/2;
		double dist=sqrt( x*x + (pos_y-y)*(pos_y-y) );
		double height = pos_z;
		double ans=atan2(height, dist)*180/_PI;
